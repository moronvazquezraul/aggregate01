...
db.encargos.aggregate(
        ...
        {$group:
            {nombre: "Jose Manuel Romero Romero", total: {$sum: {$divide: ["$preciototal","$cantidadunidades"]}}}}
    
)
//Muestra la media de dinero ganado por unidad vendida.



db.ventas.aggregate([
    {$match:
        {nombre: /Jose Manuel Romero Romero/i},
    },
    {$group:
        {promedio: {$avg: "$preciototal"}}
    },
])  

//Media de productos vendidos por Jose Manuel

db.ventas.aggregate([
    {$match:
        {nacionalidad:/España/i}},
    {$group:
        {minimo: {$min: "$preciototal"}, maximo: {$max: "$preciototal"}}
    },
])